...
impl AuthService {
    ...
    /// ログイン処理
    pub async fn login(&self, email: String, password: String) -> Result<(String, String, UserResponse)> {
        let req = UserLoginRequest { email: email.clone(), password };
        ...
        // トークン発行
        let roles = vec![Role::EditMemo, Role::ViewMemo, Role::SummarizeMemo, Role::EditTag, Role::EditAccount];
    ...
    async fn send_verification_code_internal(
        ...
        match purpose {
            VerificationPurpose::Registration => {
                self.email_service.send_verification_code(email, &verification_code).await?
            }
            VerificationPurpose::PasswordReset => {
                self.email_service.send_password_reset_code(email, &verification_code).await?
            }
        }
    ...
    pub async fn verify_email_and_issue_registration_token(&self, email: String, code: String) -> Result<String> {
        // 認証コードを検証
        let is_valid = self.verification_store.verify_code(&email, &code, &VerificationPurpose::Registration)
            .map_err(|e| AppError::ValidationError(e))?;
        
        if !is_valid {
            return Err(AppError::ValidationError("Verification code is invalid".to_string()));
        }
        ...
        self.verification_store.store_registration_token(email, registration_token.clone());
    ...
    pub async fn complete_registration(
        ...
        // インメモリストアで使用済みチェック
        self.verification_store.verify_registration_token(&registration_token, &user.email)
            .map_err(|e| AppError::ValidationError(e))?;
        
        // メールアドレスの重複チェック（二重チェック）
        if self.auth_repo.find_user_by_email(&user.email).await?.is_some() {
            return Err(AppError::ValidationError("Email address is already in use".to_string()));
        }
    ...
    /// レガシー: 即座にユーザー登録（開発用）
    #[deprecated(note = "メール認証フローを使用してください: start_registration -> verify_email_and_issue_registration_token -> complete_registration")]
    pub async fn register_immediate(&self, user: UserCreateRequest) -> Result<(String, String, UserResponse)> {
        // メールアドレスの重複チェック
        if self.auth_repo.find_user_by_email(&user.email).await?.is_some() {
            return Err(AppError::ValidationError("Email address is already in use".to_string()));
        }
    ...
    pub async fn verify_reset_code_and_issue_reset_token(&self, email: String, code: String) -> Result<String> {
        // 確認コードを検証
        let is_valid = self.verification_store.verify_code(&email, &code, &VerificationPurpose::PasswordReset)
            .map_err(|e| AppError::ValidationError(e))?;
        ...
        // インメモリストアに保存（使用済みチェック用）
        self.verification_store.store_registration_token(email, reset_token.clone());
    ...
    pub async fn complete_password_reset(&self, reset_token: &str, email: &str, new_password: &str) -> Result<()> {
        ...
        // インメモリストアで使用済みチェック
        self.verification_store.verify_registration_token(reset_token, email)
            .map_err(|e| AppError::ValidationError(e))?;
        ...
        // 新しいパスワードを設定
        self.auth_repo.reset_password(&user.user_id, new_password).await?;
        
        // リセットトークンを無効化
        self.verification_store.invalidate_registration_token(reset_token);
